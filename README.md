...
Keyball39 是一块 39 键的无线分体键盘：nice!nano 主控、内置轨迹球，屏幕可以选
...
* 轨迹球在**左半** → `leftball`（OLED）或 `leftball_niceview`（nice!view）；
...
| `leftball_niceview` | **左半** | nice!view | 左半 | 左半球 + nice!view |
| `dualball` | **左右各一个** | OLED | 右半 | 两个轨迹球；用 efogtech 版 pixart 驱动（带 Levels54 参数） |
| `noball` | 无 | OLED | 右半 | 不装轨迹球 |
| `dongle` | 右半 | OLED | **XIAO BLE 接收器** | 用 `keyball39_dongle` + `prospector_adapter` 做中央，左右半变外设 |
...
* 默认上限：8 个宏（每个最大 256 字节）、8 个组合键（每个最多 16 个键位），要更多就
  改 `keyball39_right.conf` 里的 `ZMK_RUNTIME_MACRO_*` / `ZMK_RUNTIME_COMBO_*`。
...
| 产物 | 用途 |
| --- | --- |
| `keyball39_right.uf2`（dya / dya-nv 的产物名） | 右半 = **主手（中央）**：接 USB / 蓝牙，轨迹球在这一半 |
| `keyball39_left.uf2` | 左半 = 副手（外设） |
| `keyball39_reset.uf2` | 清空已保存的设置（键位改动、配对信息等），从固件默认值重新开始 |
...
Keyball39 is a 39-key wireless split keyboard: nice!nano controller, built-in
...
* Trackball on the **right half** (the usual right-hand-ball build) → use the DYA line,
  ...
  most features.
* Trackball on the **left half** → `leftball` (OLED) or `leftball_niceview` (nice!view);
...
| `leftball_niceview` | **left** | nice!view | left | left ball + nice!view |
...
| `dongle` | right | OLED | **XIAO BLE receiver** | `keyball39_dongle` + `prospector_adapter` as the central, both halves as peripherals |
...
* Defaults: 8 macros (256 bytes each) and 8 combos (16 positions each) — tune them with
  `ZMK_RUNTIME_MACRO_*` / `ZMK_RUNTIME_COMBO_*` in `keyball39_right.conf`.
...
| Artifact | Purpose |
| --- | --- |
| `keyball39_right.uf2` (dya / dya-nv artifact name) | Right half = **main hand (central)**: USB/BLE to the host, and the trackball |
| `keyball39_left.uf2` | Left half = peripheral |
| `keyball39_reset.uf2` | Wipe stored settings (keymap edits, pairings, …) and start from firmware defaults |
...
* Battery history writes to flash periodically; turn `CONFIG_ZMK_BATTERY_HISTORY*` off in
  `keyball39_right.conf` if you don't need it.
* `dya` / `dya-nv` have moved from the ZMK v0.3 line to `main+dya` (ZMK main /
  ...
  **flash `keyball39_reset.uf2` first when switching between the two lines**.
