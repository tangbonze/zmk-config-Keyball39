...
模块（默认未启用），需要时在 `config/boards/shields/keyball_nano/keyball39_right.conf`
里加对应的 `CONFIG_*`。
...
| 产物 | 用途 |
| --- | --- |
| `nice_nano_v2__keyball39_right_nice_view_adapter_nice_view.uf2` | 右半 = **主手（中央）**：接 USB / 蓝牙，轨迹球在这一半 |
| `nice_nano_v2__keyball39_left_nice_view_adapter_nice_view.uf2` | 左半 = 副手（外设） |
...
`config/boards/shields/keyball_nano/keyball39_right.conf` if you want them.
...
| Artifact | Purpose |
| --- | --- |
| `nice_nano_v2__keyball39_right_nice_view_adapter_nice_view.uf2` | Right half = **main hand (central)**: USB/BLE to the host, and the trackball |
| `nice_nano_v2__keyball39_left_nice_view_adapter_nice_view.uf2` | Left half = peripheral |
