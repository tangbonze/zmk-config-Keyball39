...
Keyball39 是一块 39 键的无线分体键盘，自带轨迹球和 nice!view OLED 屏幕，属于
...
`dya-nv` 分支把 Keyball39 接到 **DYA Studio**（cormoran 的 ZMK Studio 增强版）上：
...
| 板级 | `nice_nano_v2` + `keyball39` shield | 同左（结构未变） |
...
等模块，但默认没有启用；需要时在 `config/boards/shields/keyball_nano/keyball39_right.conf`
里加上对应的 `CONFIG_*` 即可。
...
* 电池历史会周期性写入 flash，不需要的话可以把 `keyball39_right.conf` 里的
...
Keyball39 is a 39-key wireless split keyboard with an integrated trackball and a nice!view
...
The `dya-nv` branch makes Keyball39 work with **DYA Studio** (cormoran's enhanced ZMK
Studio):
...
| Board | `nice_nano_v2` + `keyball39` shield | same (structure unchanged) |
...
`CONFIG_*` to `config/boards/shields/keyball_nano/keyball39_right.conf` if you want them.
